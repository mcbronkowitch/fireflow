...
// P_CHOKE: the terrain draws NO choke -- a single-point span, the same shape and
// the same reason as P_PACE below. The row exists so the base overlay has a
// destination (generate() applies the overlay by walking kBaseRules), and
// draw_span returns the centre exactly when lo == hi.
//
// WHY A GENERATOR MAY NOT DRAW THIS AT ALL. CHOKE's inhibit is binary at every
// stage: instrument.cpp sets `_parts[yld].set_inhibit(window)` and
// Part::_note_suppressed then swallows every note that deck would start. The
// magnitude is never consulted -- only the sign and `amt > 0` -- so there is no
// gradient here for a draw to sit on, and any non-zero value is a deck mute
// conditioned on the other deck's gate. In the free mode it is not even
// conditioned: `window = gate() || flow()` with Part::flow() == !_step_on, so
// the window never closes.
//
// This row used to be a continuous {-.25, .25}, which never lands on 0. The
// measured cost: 45 of 52 terrains played on ONE deck, 31 of 31 in the free
// mode (docs/2026-08-13-glow-macro-audit.md). A zone-snapped draw was built
// first and is recorded there too -- it fixed the free mode but still handed a
// mute to any terrain whose priority deck sustains, and drawing the SIGN pointed
// the priority at the texture deck half the time.
//
// So CHOKE reaches Glow through a transferred patch and nowhere else. That path
// is real, not theoretical: flow_patch_bridge.hpp:569 carries it as
// `P_CHOKE = CHOKE * 0.5`, which lands Fireflow's five snapped by-ear states on
// flow's -1..1 unrounded, and the overlay is applied AFTER this loop so it wins.
{ P_CHOKE,    {{0.f,0.f},{0.f,0.f},{0.f,0.f},{0.f,0.f}} },  // overlay only
