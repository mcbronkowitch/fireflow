...
template <class OscT>
void VoiceT<OscT>::init(float sample_rate, uint32_t seed) {
    _sr = sample_rate;
...
template <class OscT>
void VoiceT<OscT>::set_morph(float m) {
    _osc_a.set_morph(m);
    _osc_b.set_morph(m);
}

template <class OscT>
void VoiceT<OscT>::set_detune_cents(float max_ct) { _detune_ct = max_ct; }
template <class OscT>
void VoiceT<OscT>::set_sub_level(float n)   { _sub_level = clampf(n, 0.f, 1.f); }
template <class OscT>
void VoiceT<OscT>::set_cutoff_hz(float hz)  { _filt.SetFreq(clampf(hz, 20.f, 0.3f * _sr)); }
template <class OscT>
void VoiceT<OscT>::set_resonance(float n)   { _filt.SetRes(clampf(n, 0.f, 0.95f)); }
template <class OscT>
void VoiceT<OscT>::set_pan(float pan)       { _pan_base = clampf(pan, -1.f, 1.f); }
template <class OscT>
void VoiceT<OscT>::set_drift_amount(float a){ _drift_amt = clampf(a, 0.f, 1.f); }

template <class OscT>
void VoiceT<OscT>::_apply_freq() {                // control-rate (std::pow inside)
...
template <class OscT>
void VoiceT<OscT>::update_control(float dt_s) {
...
template <class OscT>
void VoiceT<OscT>::process(float& accL, float& accR) {
