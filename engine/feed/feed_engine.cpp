...
// The Hz law itself: its own clamp (the engine's bounds, not a knob's) and
// its own expf. init() is its only caller now, forcing _damp_hz open first
// so a re-init() at a new sample rate recomputes the coefficient even though
// the cutoff in Hz did not move.
