...
TEST_CASE("glow toggle maps snapped positions and clamps out-of-range values") {
    CHECK(panel::switchFrameIndex(-100.f) == 0);
    CHECK(panel::switchFrameIndex(-1.f) == 0);
    CHECK(panel::switchFrameIndex(-0.51f) == 0);
    CHECK(panel::switchFrameIndex(-0.49f) == 1);
    CHECK(panel::switchFrameIndex(0.f) == 1);
    CHECK(panel::switchFrameIndex(0.49f) == 1);
    CHECK(panel::switchFrameIndex(0.51f) == 2);
    CHECK(panel::switchFrameIndex(1.f) == 2);
    CHECK(panel::switchFrameIndex(100.f) == 2);
}

TEST_CASE("one missing required layer selects fallback without changing 16 HP") {
