...
- **Project status, 2026-08-04:** the project continues as a **standalone
  ...
  **2026-08-09: one-in-one-out is retired too.** The control-reduction round
  ...
  `params=69 inputs=12 outputs=6 lights=19 panel=60HP` (measured 2026-08-16).
...
**DPTH is the MOTION lane's base on all six engines.** The mechanism already
existed — every engine already read `LANE_MOTION` as something (width+drift
on SYNTH/WAVE, drift alone on BODY, scatter on the sampler, the feedback
amount on BBD, the FM index on FEED) — the host simply never wrote the base,
so the FEED-only ternary in `Fireflow.cpp` is gone and
`inst.set_target_base(p, LANE_MOTION, pp(DEPTH_A, p))` runs unconditionally.
Five of the six cells cost no engine code; the sampler is the one exception,
reading the base scaled by `sampler_cfg::kMotionBaseScale = 0.5f` rather than
...
**One thing open, honestly: PULL has had no listening pass.**
`kPullDead = 0.03` and the linear-after-dead-zone probability curve are both
first-try values (`docs/by-ear-decisions.md`).

**The hardware placement is settled (2026-08-23), and how it got there is worth
recording.** PULL first shipped *loose*, outside every group frame, because a
0.1 mm brute-force scan found no clearing position for a 7th S-class knob
anywhere in the centre column (ROOM best margin −1.201 mm; GLOBAL −0.945 mm;
TIMING's +0.100 mm is fabrication noise, not a slot). That scan asked whether a
slot existed **without moving anything** — the honest answer to a question
narrower than the real one. Re-pitching a row was the way in all along; it was
ruled out as a layout decision an implementer should not take alone, and
Bastian took it a day later.

PULL is now the **fourth knob in the GLOBAL centre row**, beside CHOKE — the
only other bipolar control in the centre, and the one whose sign convention
PULL was designed on. The row keeps its 13.0 mm pitch and its 1.0 mm body gap
unchanged; the three existing knobs each moved 6.5 mm left so the group of four
re-centres on 152.40. Measured: bodies span 126.90–177.90 inside a cell of
123.00–181.80, so 3.90 mm spare on each side.

Two deliberate costs. With an even count no knob sits on the centre line any
more, and GLOBAL no longer flushes with TEMPO/COUPLE/SHUFFLE in the TIMING row
below (ROOM already breaks that flush with its 16.0 mm pitch). Re-centring was
not optional: leaving SCALE at 139.40 and hanging PULL off the right end lands
it at 178.40, whose body overruns the cell edge by 2.60 mm.

The upshot for the plate as a whole: **"60 HP with room to spare" survives, but
only through re-pitching.** There is still no slot that costs nothing — the
...
2026-08-19 on the same probe that found FEED's +11.8 dB and fixed it. One
`Part`, identical lanes, identical LEVEL 1, no part FX and no reverb, 6 s from
one trigger:

| engine | FLOW peak | FLOW RMS vs SYNTH | STEP RMS vs SYNTH |
|---|---:|---:|---:|
| SYNTH | 0.333 | ±0 | ±0 |
| WAVE | 0.444 | +2.8 dB | +3.4 dB |
| FEED (after the trim) | 0.369 | +3.1 dB | +8.4 dB |
| **BODY** | **0.053** | **−10.0 dB** | **−29.1 dB** |

Under the parity brief — *"die sollen gleich laut sein damit man auch im
laufenden Betrieb die Engine wechseln kann ohne dass es krasse
Lautstärke-Ausreißer gibt"* — that is the same defect FEED had, mirrored. It
was found during the FEED round and deliberately **not** fixed there.

**Why it is its own session and not a one-line trim.** BODY's level rides MATL
and EXCIT hard, so a single probe setting is a reading, not a verdict: the
29 dB struck figure in particular may be as much about the excitation the probe
happens to use as about the engine's gain. FEED's fix was a single output trim
because its whole deck is one summed bank; BODY has a resonator whose output
depends on what is exciting it, and trimming the wrong end would flatten the
dynamic range the engine exists for. The round needs its own probe sweep across
MATL and EXCIT before anything moves.

Starting points: the per-engine table and the `kDeckGain` reasoning in
[`engine-map.md`](engine-map.md) §9, the parity brief in
[`by-ear-decisions.md`](by-ear-decisions.md), and the scratchpad level probe
...
- **The Rack panel's fourth VOICE column still sits off-pitch**, unrelated to
  ...
  column on pitch (40.75) fails by 2.5 mm. The hardware plate has no such
  problem: there the row is a clean 13 mm 4×2, the same pitch as
  ATK/DEC/RES/SUB above it. Nothing in this round moved a column, and nothing
  here fixes it.
...
### M5l — PULL ✅ built, HW placement settled, ⏳ no listening pass
...
1. **Panel design** — decide the prototype's control surface. The reduction
   ...
   `params=69 inputs=12 outputs=6 lights=19 panel=60HP` today, and that printed
...
**2026-09-02 — the coupon layout is generated, proven ten ways, and Gerbers
exist.** `hardware/coupon/` now carries the physical board alongside the
schematic: `scripts/placement.py`/`routing.py` hold every part's position
and every signal track as data, `scripts/check_layout.py` measures the
layout spec's five analog rules from the saved artifact, and
`scripts/build_pcb.py` is the proof chain -- net comparison, courtyard and
module-shadow placement sanity, plane connectivity, stitch-copper hygiene,
...
the pin" (their only F.Cu approach also carries the 8:1 mux's enable route),
and five of the six address/enable-bus nets cross the moat 70-74 mm from the
star point `JP_GND` instead of beside it, because they originate at `U_SR1`
in the far-west digital column rather than near `JP_GND` on the far east.
Both share the same unattempted fix -- re-pitching the 8:1's column and its
west approach. **What still blocks the order is unchanged and needs hands,
...
`feat/mod-latch-layer`. **Checked interactively in Rack 2026-08-22** — Bastian
latched MOD on `FireflowHW` and confirmed the swap, the booted depths and the
