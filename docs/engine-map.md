...
### The one open question that got a knob

Neither DEPTH nor the in-loop DAMP cutoff had a panel home, and both are
recorded above as unanswered. DEPTH spent 2026-08-19 as a context-menu slider
and is a **panel knob** since the same evening — `DPTH`, in VOICE on both
panels, one per deck.
...
- **DAMP cutoff** spent the same evening first as an absolute 200 Hz–16 kHz
  LOG control, then — later the same day — as a bipolar trim broadcasting to
  all six engines: `EDGE`, the panel's fourth VOICE knob. It shipped
  2026-08-19 and was **withdrawn 2026-08-20** after the listening pass this
  section used to record as owed — see `docs/by-ear-decisions.md` for the
  measurements and the reasoning. FEED's own DAMP corner is once again a
  fixed `feed_cfg::kDampFixedHz` = 3200 Hz with no panel reach, exactly as
  before EDGE existed; the other five engines' second filters lost their trim
  along with it. The VOICE slot is free space by decision, not re-flowed.

DPTH boots neutral, so a patch that never touches it behaves exactly as
before. As a ParamId Rack persists it for free, so the module holds no state
for it at all — the float array, its JSON key and its reset all left with the
menu.
...
because that knob IS the `LANE_MOTION` base.
...
## 10. DPTH across the six engines
...
FEED-only ternary). The spec and plan shipped a second knob alongside it,
EDGE — a bipolar trim on each engine's own "second filter" — the same day;
EDGE was withdrawn 2026-08-20 after a listening pass (`docs/by-ear-decisions.md`),
and this section no longer describes it.
...
EDGE used to have its own subsection here — the per-engine neutral/span
table, the negative-half measurements, and BODY's zone-2 blind spot. All of
