...
- **flow param** — the `spky::flow::ParamId` (`engine/flow/flow_params.h:48-89`).
  Rows are in that enum's order.
...
2. **The knob's value is not always the engine's value.** For `ENGINE`, `CHOKE`,
   ...
   (`flow_params.h:114`) calls the *same* engine setter with the overlay value
...
| `P_COUPLE` | `COUPLE` (printed **`FREE\|GRID`**) | **formula, zone split.** `k = COUPLE`, `grid = k >= 0.5` (`kCoupleZoneSplit`, `Fireflow.cpp:34`); `P_COUPLE = grid ? (k - 0.5) / 0.5 : k / 0.5` (`Fireflow.cpp:878-883`) | Store the rescaled half-zone value, **not the knob**: flow's `apply_param` hands `P_COUPLE` straight to `set_couple` (`flow_params.h:165`), which is what Fireflow's rescaled value feeds. This is also the sole source of `set_sync` in Fireflow (`:880`) — see `P_MODE` |
| `P_CHOKE` | `CHOKE` | **formula.** `P_CHOKE = CHOKE * 0.5` (`Fireflow.cpp:898`) | The knob is `configSwitch(c.id, -2.f, 2.f, …)` with 5 snapped states (`:320-324`); x0.5 lands them on flow's `-1..1` (`flow_params.h:75`). The five states are by-ear rulings (`spotykach-by-ear-decisions`) — the halving is the whole conversion, nothing rounds |
...
| `P_TEMPO_BPM` | `TEMPO` | **formula, then clamp.** `bpm = 40 + TEMPO * 200` (`Fireflow.cpp:918`), i.e. **40..240 BPM**. `kParams[P_TEMPO_BPM]` is **50..140** (`flow_params.h:80`). Clamp, and **report anything outside** | An external clock at `CLOCK` overrides the knob at runtime (`:919-922`); that is live state, not patch state, and does not transfer |
