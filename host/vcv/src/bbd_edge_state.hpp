...
// The ENG-switch-into-BBD edge detector used by Spotymod::pushParams() (spec
...
// lives in its own tested unit rather than as inline bools in Spotymod.cpp:
...
//     module (right-click Load preset, module paste -- Spotymod.cpp's own
...
struct BbdEdgeState {
    ...
    // Call when a JSON restore lands on an ALREADY-LIVE instance (Spotymod's
